...
public final class MenuEditorScreen extends EzScreen {
    ...
    protected void init() {
        ...
        filterBox.setSuggestion(Component.translatable("ezactions.gui.menu_editor.hint.filter").getString());
    ...
    public void render(GuiGraphics g, int mouseX, int mouseY, float partialTick) {
        ...
        if (filterBox != null) {
            boolean over = mouseX >= filterBox.x && mouseX < filterBox.x + filterBox.getWidth()
                    && mouseY >= filterBox.y && mouseY < filterBox.y + filterBox.getHeight();
            if (over) {
                g.renderTooltip(this.font, Component.translatable("ezactions.gui.menu_editor.tooltip.filter"), mouseX, mouseY);
            }
        }
    }
