...
import org.z2six.minemenuplus.mixin.KeyboardHandlerAccessor;
...
public final class InputInjector {
    ...
    private static boolean deliverInput(Minecraft mc, int glfwKey, int glfwScanCode, int glfwMods) {
        try {
            ...
            // press now
            acc.minemenuplus$keyPress(window, glfwKey, glfwScanCode, GLFW.GLFW_PRESS, glfwMods);

            // release next tick
            ClientTaskQueue.post(() -> {
                try {
                    acc.minemenuplus$keyPress(window, glfwKey, glfwScanCode, GLFW.GLFW_RELEASE, glfwMods);
                } catch (Throwable t) {
